...
class RepositoryManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path:
            ...
            if env_config:
                ...
                # Default location
                self.config_path = Path.home() / ".local" / "share" / "github-agent" / "repositories.json"
    ...
    def load_configuration(self) -> bool:
        ...
        try:
            ...
            if self.config_path.exists():
                self.logger.info(f"Loading repository configuration from {self.config_path}")
                with open(self.config_path, 'r') as f:
                    config_data = json.load(f)
                
                self._parse_configuration(config_data)
                self._validate_repositories()
                self.logger.info(f"Successfully loaded {len(self.repositories)} repositories")
                return True
            else:
                # Try fallback to single repository mode
                self.logger.info("No multi-repo config found, attempting single-repo fallback")
    ...
    def _parse_configuration(self, config_data: Dict) -> None:
        ...
        for name, repo_data in repositories_data.items():
            if not isinstance(repo_data, dict):
                raise ValueError(f"Repository '{name}' configuration must be a dictionary")
            
            required_fields = ["path"]
            for field in required_fields:
                if field not in repo_data:
                    raise ValueError(f"Repository '{name}' missing required field: {field}")
        ...
        for name, repo_config in self.repositories.items():
            try:
                ...
                if not os.access(repo_config.path, os.R_OK):
                    raise ValueError(f"No read access to repository: {repo_config.path}")
                
                self.logger.debug(f"Validated repository '{name}' at {repo_config.path}")
    ...
    def _load_fallback_configuration(self) -> bool:
        ...
        if not repo_path:
            self.logger.error("No repository configuration found and LOCAL_REPO_PATH not set")
            return False
        ...
        try:
            ...
            self.logger.info(f"Using fallback single repository mode: {repo_config.path}")
            return True
