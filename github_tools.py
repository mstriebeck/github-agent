...
async def execute_read_build_logs(
    ...
    try:
        ...
        if build_id is None:
            return json.dumps({"error": "Could not find build ID"})

        if context.repo_name is None:
            return json.dumps({"error": "Repository name not available in context"})
        ...
        try:
            ...
            return json.dumps({"error": f"Could not find build artifact: {e}"})
        ...
        try:
            ...
            return json.dumps({"error": f"Could not download artifact: {e}"})
