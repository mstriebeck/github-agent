...
class ExitCodeManager:
    ...
    def report_verification_failure(self, check: str, details: str):
        ...
        if "zombie" in check.lower():
            self._verification_failures.append(ShutdownExitCode.ZOMBIE_PROCESSES_DETECTED)
    ...
    def determine_exit_code(self, shutdown_reason: str = "manual") -> ShutdownExitCode:
        ...
        if self._verification_failures:
            ...
            self.logger.critical(f"Exiting with critical failure code: {critical_code.name} ({critical_code})")
            return critical_code
            
        # System errors next
        if self._issues:
            error_code = max(self._issues)
            self.logger.error(f"Exiting with error code: {error_code.name} ({error_code})")
            return error_code
            
        # Force actions indicate degraded shutdown
        if self._forced_actions:
            force_code = max(self._forced_actions)
            self.logger.warning(f"Exiting with force action code: {force_code.name} ({force_code})")
            return force_code
            
        # Clean shutdown
        if shutdown_reason in ["SIGTERM", "SIGINT", "signal"]:
            self.logger.info(f"Exiting with clean signal shutdown: {ShutdownExitCode.SUCCESS_SIGNAL_SHUTDOWN}")
            return ShutdownExitCode.SUCCESS_SIGNAL_SHUTDOWN
        else:
            self.logger.info(f"Exiting with clean manual shutdown: {ShutdownExitCode.SUCCESS_CLEAN_SHUTDOWN}")
    ...
    def get_exit_summary(self) -> dict:
        ...
        return {
            ...
            "verification_failures": [code.name for code in self._verification_failures],
            "total_problems": len(self._issues) + len(self._forced_actions) + len(self._verification_failures)
        }
