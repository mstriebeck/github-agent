...
from shutdown_core import RealProcessSpawner, ShutdownCoordinator
...
class _ResourceTracker:
    ...
    def add_database_connection(self, name: str, connection: Any) -> None:
        ...
        if self._closed:
            self.logger.warning(
                f"Cannot add database connection {name} - tracker is closed"
            )
            return
    ...
    def add_cleanup_callback(self, name: str, callback: Callable) -> None:
        ...
        if self._closed:
            self.logger.warning(
                f"Cannot add cleanup callback {name} - tracker is closed"
            )
            return
    ...
    async def cleanup_all(self) -> bool:
        ...
        if self._database_connections:
            self.logger.info(
                f"Closing {len(self._database_connections)} database connections"
            )
            for name, conn in self._database_connections.items():
                try:
                    ...
                    self.logger.error(
                        f"✗ Failed to close database connection {name}: {e}"
                    )
                    success = False
        ...
        if self._resources:
            ...
            sorted_resources = sorted(
                self._resources.items(), key=lambda x: x[1]["priority"]
            )
        ...
        if self._cleanup_callbacks:
            self.logger.info(
                f"Executing {len(self._cleanup_callbacks)} cleanup callbacks"
            )
            for name, callback in self._cleanup_callbacks:
                try:
                    ...
                    self.logger.error(
                        f"✗ Failed to execute cleanup callback {name}: {e}"
                    )
                    success = False
        ...
        status = (
            ...
            if success
            else "✗ Some resources failed to clean up"
        )
    ...
    def cleanup_resources(self) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            try:
                ...
                try:
                    open_files, connections = future.result(
                        timeout=2.0
                    )  # 2 second timeout
                    self.logger.debug(
                        f"Process has {len(open_files)} open files and {len(connections)} connections"
                    )
                    ...
                    if open_files:
                        ...
                        error_msg = (
                            f"Process has {len(open_files)} open files: {file_paths}"
                        )
...
class _ClientTracker:
        ...
        if remaining:
            ...
            for client in remaining:
                try:
                    ...
                    self.logger.error(
                        f"Failed to force disconnect {client.client_id}: {e}"
                    )
                    success = False
    ...
    def disconnect_all_clients(
        ...
        status = (
            ...
            if success
            else "✗ Some clients failed to disconnect"
        )
        self.logger.info(status)
        return success, errors
    ...
    async def _send_shutdown_notification(self, client: ClientInfo) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to send shutdown notification to {client.client_id}: {e}"
            )
            raise
...
class ShutdownManager:
    ...
    def __init__(self, logger: logging.Logger, mode: str = "master"):
        ...
        self._health_monitor = HealthMonitor(
            logger, f"/tmp/mcp_server_health_{os.getpid()}.json"
        )
    ...
    async def shutdown(
        ...
        if self._shutdown_initiated:
            self.logger.warning(
                f"Shutdown already initiated (reason: {self._shutdown_reason}), ignoring new request ({reason})"
            )
            return False
    ...
    async def _shutdown_master(self, grace_period: float, force_timeout: float) -> bool:
        ...
        self.logger.info(
            f"🎛️ Starting master shutdown (grace: {grace_period}s, force: {force_timeout}s)"
        )
        ...
        await self.system_monitor.log_system_state(
            self.logger, "MASTER_SHUTDOWN_STARTING"
        )
        ...
        try:
            ...
            if self._workers:
                self.logger.info(
                    f"👷 Phase 1: Shutting down {len(self._workers)} workers"
                )
                self._health_monitor.set_shutdown_phase(ShutdownPhase.WORKERS_STOPPING)
                worker_success = await self._shutdown_all_workers(
                    grace_period * 0.6, force_timeout
                )
                if worker_success:
                    ...
                    self.logger.warning(
                        "⚠️ Phase 1 partial: Some workers failed to shut down"
                    )
                    success = False
            else:
            ...
            client_success = await self._client_tracker.disconnect_all(
                grace_period * 0.2
            )
            if client_success:
            ...
            if resource_success:
                ...
                self.logger.warning(
                    "⚠️ Phase 3 partial: Some resources failed to clean up"
                )
                success = False
        ...
        if success:
            ...
            self.logger.error(
                f"💥 Master shutdown completed with errors in {total_duration:.3f}s"
            )
    ...
    async def _shutdown_worker(self, grace_period: float, force_timeout: float) -> bool:
        ...
        self.logger.info(
            f"👷 Starting worker shutdown (grace: {grace_period}s, force: {force_timeout}s)"
        )
        ...
        await self.system_monitor.log_system_state(
            self.logger, "WORKER_SHUTDOWN_STARTING"
        )
        ...
        try:
            ...
            if client_success:
                ...
                self.logger.warning(
                    "⚠️ Phase 1 partial: Some clients failed to disconnect"
                )
                success = False
            ...
            if resource_success:
                ...
                self.logger.warning(
                    "⚠️ Phase 2 partial: Some resources failed to clean up"
                )
                success = False
        ...
        if success:
            ...
            self.logger.error(
                f"💥 Worker shutdown completed with errors in {total_duration:.3f}s"
            )

        return success

    async def _shutdown_all_workers(
        self, grace_period: float, force_timeout: float
    ) -> bool:
        ...
        if success_count == total_workers:
            ...
            self.logger.error(
                f"✗ Only {success_count}/{total_workers} workers shut down successfully"
            )
            return False

    async def _shutdown_single_worker(
        self, worker: WorkerProcess, grace_period: float, force_timeout: float
    ) -> bool:
        ...
        try:
            ...
            try:
                    ...
                    async with aiohttp.ClientSession(
                        ...
                        async with session.post(
                            ...
                            if response.status == 200:
                                self.logger.debug(
                                    f"Sent shutdown request to worker {worker.repo_name}"
                                )
                else:
                ...
                if not worker.is_running():
                    self.logger.info(
                        f"✓ Worker {worker.repo_name} shut down gracefully"
                    )
                    return True
    ...
    def _signal_handler(self, signum, frame):
        ...
        if not self.shutdown_in_progress:
            try:
                ...
                self.logger.warning(
                    "No running event loop for signal handler, creating new event loop"
                )
                try:
                    ...
                    def run_shutdown():
                        try:
                            asyncio.run(self.shutdown(reason=f"signal_{signal_name}"))
                        except Exception as e:
                    ...
                    self.logger.error(
                        f"Failed to start shutdown in new event loop: {e}"
                    )
    ...
    async def _verify_shutdown(self) -> tuple[bool, list[str]]:
        ...
        if self._client_tracker.get_client_count() > 0:
            errors.append(
                f"{self._client_tracker.get_client_count()} clients still connected"
            )
            success = False
