...
class HealthMonitor:
    ...
    def __init__(self, logger, health_file_path: str = "/tmp/mcp_server_health.json"):
        ...
        self.logger.info(f"Health monitor initialized, writing to {self.health_file_path}")
    ...
    def set_server_status(self, status: ServerStatus):
        ...
        with self._lock:
            ...
            self._status = status
            self.logger.info(f"Server status changed: {old_status.value} -> {status.value}")
    ...
    def set_shutdown_phase(self, phase: ShutdownPhase):
        ...
        with self._lock:
            ...
            self.logger.info(f"Shutdown phase changed: {old_phase.value} -> {phase.value}")
            
    def update_worker_status(self, worker_id: str, pid: Optional[int], port: int, status: str):
    ...
    def is_shutdown_complete(self) -> bool:
        ...
        with self._lock:
            return self._shutdown_phase in [ShutdownPhase.COMPLETED, ShutdownPhase.FAILED]
    ...
    def get_stuck_workers(self, timeout_seconds: float = 30.0) -> List[str]:
        ...
        with self._lock:
            ...
            for worker_id, worker in self._workers.items():
                if (worker.shutdown_requested and 
                    not worker.shutdown_completed and 
                    worker.last_seen < cutoff_time):
...
def read_health_status(health_file_path: str = "/tmp/mcp_server_health.json") -> Optional[Dict[str, Any]]:
...
def is_server_healthy(health_file_path: str = "/tmp/mcp_server_health.json", 
                     max_age_seconds: float = 10.0) -> bool:
