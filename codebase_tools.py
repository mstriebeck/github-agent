...
def validate(logger: logging.Logger, repositories: dict[str, Any]) -> None:
    ...
    logger.info(f"✅ Codebase service validation passed for {len(repositories)} repositories")


def _validate_workspace_access(logger: logging.Logger, workspace: str, repo_name: str) -> None:
    """
    ...
    if not os.path.isdir(workspace):
        raise RuntimeError(f"Repository workspace is not a directory: {workspace} (repo: {repo_name})")

    if not os.access(workspace, os.R_OK):
        raise RuntimeError(f"Repository workspace is not readable: {workspace} (repo: {repo_name})")

    if not os.access(workspace, os.W_OK):
        raise RuntimeError(f"Repository workspace is not writable: {workspace} (repo: {repo_name})")
...
def _validate_python_lsp_tools(logger: logging.Logger, repo_name: str) -> None:
    ...
    try:
        ...
        logger.debug(f"Python LSP tools validation passed for {repo_name}: pyright {version}")
