...
class ShutdownCoordinator:
    ...
    def __init__(self, logger):
        ...
        self._shutdown_initiated = (
            False  # Flag to prevent multiple shutdown initiations
        )
    ...
    def shutdown(self, reason="manual"):
        ...
        if self._shutdown_initiated:
            self.logger.warning(
                f"Shutdown already initiated (reason: {self._shutdown_reason}), ignoring new request"
            )
            return
...
def setup_signal_handlers(shutdown_coordinator):
    ...
    def signal_handler(signum, frame):
        ...
        if (
            shutdown_coordinator._shutdown_initiated
        ):  # Use the coordinator's internal flag
            shutdown_coordinator.logger.warning(
                f"Already handling shutdown, ignoring signal {signum}"
            )
            return

        signal_name = signal.Signals(signum).name
        shutdown_coordinator.logger.critical(
            f"Received signal {signum} ({signal_name})"
        )
        shutdown_coordinator.logger.debug(
            f"Signal received in frame: {frame.f_code.co_filename}:{frame.f_lineno}"
        )
