...
class DatabaseConnectionManager:
    ...
    def add_connection_pool(self, name: str, pool: Any) -> None:
        ...
        if self._closed:
            self.logger.warning(f"Cannot add connection pool {name} - manager is closed")
            return
    ...
    async def close_all_connections(self, timeout: float = 30.0) -> bool:
        ...
        self.logger.info(f"Closing {len(self._connections)} connections and {len(self._connection_pools)} pools")
        ...
        for name, connection in list(self._connections.items()):
            try:
                ...
                self.logger.info(f"✓ Closed database connection {name} in {conn_duration:.3f}s")
                
            except Exception as e:
                conn_duration = (datetime.now() - conn_start).total_seconds()
                self.logger.error(f"✗ Failed to close database connection {name} after {conn_duration:.3f}s: {e}")
                success = False
        ...
        for name, pool in list(self._connection_pools.items()):
            try:
                ...
                self.logger.info(f"✓ Closed database connection pool {name} in {pool_duration:.3f}s")
                
            except Exception as e:
                pool_duration = (datetime.now() - pool_start).total_seconds()
                self.logger.error(f"✗ Failed to close database connection pool {name} after {pool_duration:.3f}s: {e}")
                success = False
        ...
        status = "✓ All database connections closed" if success else "✗ Some database connections failed to close"
    ...
    def close_all_files(self) -> bool:
        ...
        for name, file_handle in list(self._file_handles.items()):
            try:
                ...
                self.logger.info(f"✓ Closed file handle {name} in {handle_duration:.3f}s")
                
            except Exception as e:
                handle_duration = (datetime.now() - handle_start).total_seconds()
                self.logger.error(f"✗ Failed to close file handle {name} after {handle_duration:.3f}s: {e}")
                success = False
        ...
        total_duration = (datetime.now() - start_time).total_seconds()
        status = "✓ All file handles closed" if success else "✗ Some file handles failed to close"
        self.logger.info(f"{status} in {total_duration:.3f}s")
...
class ExternalServiceManager:
    ...
    def add_service(self, name: str, service: Any, cleanup_func: Optional[Callable] = None) -> None:
    ...
    async def close_all_services(self, timeout: float = 20.0) -> bool:
        ...
        for name, service_info in list(self._services.items()):
            try:
                ...
                self.logger.info(f"✓ Closed external service {name} in {service_duration:.3f}s")
                
            except Exception as e:
                service_duration = (datetime.now() - service_start).total_seconds()
                self.logger.error(f"✗ Failed to close external service {name} after {service_duration:.3f}s: {e}")
                success = False
        ...
        total_duration = (datetime.now() - start_time).total_seconds()
        status = "✓ All external services closed" if success else "✗ Some external services failed to close"
        self.logger.info(f"{status} in {total_duration:.3f}s")
...
class ResourceManager:
    ...
    def add_resource(self, name: str, resource: Any, cleanup_func: Optional[Callable] = None,
                    is_async: bool = False, priority: int = 0, timeout: float = 10.0) -> None:
        ...
        with self._lock:
            ...
            self.logger.debug(f"Added resource: {name} (priority: {priority}, async: {is_async})")
    ...
    def add_database_connection(self, name: str, connection: Any) -> None:
        """Add a database connection to be managed"""
        if self._closed:
            self.logger.warning(f"Cannot add database connection {name} - manager is closed")
            return
    ...
    def add_external_service(self, name: str, service: Any, cleanup_func: Optional[Callable] = None) -> None:
    ...
    async def cleanup_all_resources(self, timeout: float = 60.0) -> int:
        ...
        try:
            ...
            services_success = await asyncio.wait_for(
                self.service_manager.close_all_services(),
                timeout=15.0
            )
            ...
            if self._resources:
                self.logger.info(f"Phase 3: Closing {len(self._resources)} generic resources")
            ...
            if self._cleanup_callbacks:
                self.logger.info(f"Phase 4: Executing {len(self._cleanup_callbacks)} cleanup callbacks")
            ...
            self.logger.error(f"Unexpected error during resource cleanup: {e}", exc_info=True)
        ...
        if not failed_resources:
            self.logger.info(f"✓ All resources cleaned up successfully in {total_duration:.3f}s")
            return ExitCodes.SUCCESS
        else:
            self.logger.error(f"✗ Resource cleanup completed with failures in {total_duration:.3f}s: {failed_resources}")
    ...
    async def _close_generic_resources(self) -> bool:
        ...
        for resource_info in sorted_resources:
            try:
                ...
                resource_duration = (resource_info.closed_at - resource_start).total_seconds()
                self.logger.info(f"✓ Closed resource {resource_info.name} in {resource_duration:.3f}s")
                
            except Exception as e:
                resource_duration = (datetime.now() - resource_start).total_seconds()
                self.logger.error(f"✗ Failed to close resource {resource_info.name} after {resource_duration:.3f}s: {e}")
                success = False
    ...
    async def _execute_cleanup_callbacks(self) -> bool:
        ...
        for callback in self._cleanup_callbacks:
            try:
                ...
                self.logger.info(f"✓ Executed cleanup callback {callback.__name__} in {callback_duration:.3f}s")
                
            except Exception as e:
                callback_duration = (datetime.now() - callback_start).total_seconds()
                self.logger.error(f"✗ Failed to execute cleanup callback {callback.__name__} after {callback_duration:.3f}s: {e}")
                success = False
