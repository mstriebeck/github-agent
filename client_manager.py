...
class MCPClient:
    ...
    def set_state(self, state: ClientState) -> None:
        ...
        with self._lock:
            ...
            self.logger.debug(
                f"Client {self.client_id} state: {old_state.value} → {state.value}"
            )
    ...
    async def send_notification(self, method: str, params: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if hasattr(self.transport, "write"):
                ...
                self.logger.warning(
                    f"Unknown transport type for client {self.client_id}"
                )
                return False
            ...
            self.logger.error(
                f"Failed to send notification to client {self.client_id}: {e}"
            )
            self.increment_errors()
            return False

    async def send_shutdown_notification(
        self, reason: DisconnectionReason, grace_period: float
    ) -> bool:
...
class ClientConnectionManager:
    ...
    def remove_client(
        self,
        client_id: str,
        reason: DisconnectionReason = DisconnectionReason.CLIENT_REQUEST,
    ) -> bool:
    ...
    def get_clients_by_group(self, group: str) -> list[MCPClient]:
        ...
        with self._lock:
            ...
            return [
                self._clients[cid]
                for cid in self._client_groups[group]
    ...
    async def graceful_shutdown(
        self, grace_period: float = 10.0, force_timeout: float = 5.0
    ) -> bool:
        ...
        self.logger.info(
            f"Starting graceful client shutdown (grace: {grace_period}s, force: {force_timeout}s)"
        )

        # Log system state before shutdown
        await self._system_monitor.log_system_state(
            self.logger, "CLIENT_SHUTDOWN_STARTING"
        )
        ...
        self.logger.info(
            f"Sent shutdown notifications to {successful_notifications}/{len(clients)} clients"
        )
        ...
        while time.time() < grace_end_time:
            ...
            self.logger.debug(
                f"Waiting for {len(remaining_clients)} clients to disconnect"
            )
        ...
        if remaining_clients:
            self.logger.info(
                f"Phase 3: Force disconnecting {len(remaining_clients)} clients"
            )
        ...
        if success:
            self.logger.info(
                f"✓ All clients disconnected successfully in {total_duration:.3f}s"
            )
        else:
            self.logger.error(
                f"✗ {len(final_clients)} clients failed to disconnect in {total_duration:.3f}s"
            )

        # Log final system state
        await self._system_monitor.log_system_state(
            self.logger, "CLIENT_SHUTDOWN_COMPLETED"
        )
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "groups": {
                    name: list(clients) for name, clients in self._client_groups.items()
                },
