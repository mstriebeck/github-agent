...
class WorkerManager:
    ...
    def __init__(self, logger, process_spawner: Optional[IProcessSpawner] = None):
        ...
        self.default_worker_timeout = float(os.getenv('MCP_SHUTDOWN_WORKER_TIMEOUT', '30'))
        self.default_graceful_timeout = float(os.getenv('MCP_WORKER_GRACEFUL_TIMEOUT', '10'))
        self.logger.debug(f"Worker timeouts - graceful: {self.default_graceful_timeout}s, total: {self.default_worker_timeout}s")
    ...
    def add_worker(self, worker: WorkerProcess) -> None:
        ...
        self.logger.info(f"Added worker {worker.repo_name} to manager (port: {worker.port})")
    ...
    def start_worker(self, worker: WorkerProcess, command: List[str], env: Optional[Dict] = None) -> bool:
        """Start worker with process group for better process management (macOS/Linux compatible)"""
        start_time = datetime.now()
        self.logger.info(f"Starting worker on port {worker.port} for {worker.repo_name}")
        
        try:
            # Check if port is available before starting
            if not self._is_port_available(worker.port):
                self.logger.error(f"Port {worker.port} is not available for {worker.repo_name}")
                return False
            ...
            self.logger.info(f"Worker started for {worker.repo_name} on port {worker.port} with PID {process.pid} in {duration:.2f}ms")
            ...
            duration = (datetime.now() - start_time).total_seconds() * 1000
            self.logger.error(f"Failed to start worker for {worker.repo_name} on port {worker.port} after {duration:.2f}ms: {e}")
            return False
    ...
    async def shutdown_all_workers(self) -> bool:
        ...
        for i, (repo_name, worker) in enumerate(list(self.workers.items())):
            task = self._shutdown_single_worker_async(repo_name, worker, i + 1, worker_count)
        ...
        # Process results
        for i, (repo_name, result) in enumerate(zip(self.workers.keys(), results)):
            if isinstance(result, Exception):
                self.logger.error(f"Exception during worker {repo_name} shutdown: {result}")
        ...
        self.logger.info(f"Worker shutdown completed: {successful_shutdowns}/{worker_count} successful in {total_duration:.3f}s")
    ...
    async def _shutdown_single_worker_async(self, repo_name: str, worker: WorkerProcess, 
                                          worker_num: int, total_workers: int) -> bool:
        """Enhanced worker shutdown with graceful protocol and process group handling"""
        start_time = datetime.now()
        self.logger.info(f"Starting enhanced shutdown sequence for worker {worker_num}/{total_workers}: {repo_name} (PID: {worker.process.pid if worker.process else 'None'})")
        
        try:
            if not worker.process or self.process_spawner.get_process_poll_status(worker.process) is not None:
                self.logger.info(f"Worker {repo_name} already stopped")
                return True
            
            # Pre-shutdown state logging
            self.logger.debug(f"Worker process state - PID: {worker.process.pid}, Poll: {self.process_spawner.get_process_poll_status(worker.process)}")
            
            # Phase 1: Attempt graceful shutdown via HTTP/IPC
            graceful_start = datetime.now()
            self.logger.debug(f"Phase 1: Requesting graceful shutdown for worker {repo_name} on port {worker.port}")
            
            try:
                await self._send_worker_shutdown_request(worker.port)
                self.logger.debug(f"Shutdown request sent to worker {repo_name} on port {worker.port}")
                ...
                try:
                    ...
                    self.logger.info(f"Worker {repo_name} shut down gracefully in {graceful_duration:.3f}s")
                    await self._comprehensive_worker_verification(worker)
                    return True
                except asyncio.TimeoutError:
                    graceful_duration = (datetime.now() - graceful_start).total_seconds()
                    self.logger.info(f"Worker {repo_name} didn't respond to graceful shutdown after {graceful_duration:.3f}s")
                    
            except Exception as e:
                graceful_duration = (datetime.now() - graceful_start).total_seconds()
                self.logger.debug(f"Graceful shutdown request failed for {repo_name} port {worker.port} after {graceful_duration:.3f}s: {e}")
            ...
            if self.process_spawner.get_process_poll_status(worker.process) is None:
                ...
                self.logger.debug(f"Phase 2: Sending SIGTERM to worker {worker.process.pid}")
                ...
                try:
                    ...
                    self.logger.info(f"Worker {repo_name} terminated gracefully after {elapsed:.3f}s")
                except asyncio.TimeoutError:
                    elapsed = (datetime.now() - sigterm_start).total_seconds()
                    self.logger.warning(f"Worker {repo_name} didn't respond to SIGTERM after {elapsed:.3f}s")
            
            # Phase 3: SIGKILL if still running
            if self.process_spawner.get_process_poll_status(worker.process) is None:
                sigkill_start = datetime.now()
                self.logger.warning(f"Phase 3: Escalating to SIGKILL for worker {repo_name}")
                
                # Try to kill the entire process group first
                try:
                    self.process_spawner.kill_process_group(worker.process.pid)
                    self.logger.debug(f"Sent SIGKILL to process group for worker {worker.process.pid}")
                except (ProcessLookupError, PermissionError) as e:
                    self.logger.debug(f"Process group kill failed for worker {worker.process.pid}: {e}")
                ...
                self.logger.info(f"Worker {repo_name} force-killed successfully in {kill_duration:.3f}s")
            
        except Exception as e:
            error_duration = (datetime.now() - start_time).total_seconds()
            self.logger.error(f"Exception during worker {repo_name} shutdown after {error_duration:.3f}s: {e}", exc_info=True)
            return False
        ...
        self.logger.debug(f"Worker {repo_name} total shutdown time: {total_duration:.3f}s (verification: {verification_duration:.3f}s)")
    ...
    async def _comprehensive_worker_verification(self, worker: WorkerProcess):
        ...
        if worker.process:
            ...
            if final_poll is None:
                self.logger.error(f"CRITICAL: Worker process {worker.process.pid} still running after shutdown!")
            else:
                self.logger.debug(f"✓ Process verification passed: worker exited with code {final_poll}")
        ...
        self.logger.info(f"Worker {worker.repo_name} verification: {verification_status} "
                        f"(total: {total_verification:.3f}s, port: {port_duration:.3f}s, "
                        f"zombie: {zombie_duration:.3f}s)")
    
    async def _verify_port_release_async(self, port: int, timeout: float = 15.0) -> bool:
        ...
        while (datetime.now() - start_time).total_seconds() < timeout:
            ...
            elapsed = (datetime.now() - start_time).total_seconds()
            self.logger.debug(f"Port {port} still in use after {elapsed:.3f}s (check took {check_duration:.2f}ms), continuing...")
    ...
    def _is_port_available(self, port: int) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Unexpected error in _is_port_available for port {port}: {e}")
            return False
            ...
            for conn in psutil.net_connections(kind='inet'):
                if conn.laddr and conn.laddr.port == port:
                    ...
                    try:
                        ...
                        self.logger.warning(f"    Cannot access process {conn.pid} details: {e}")
        except Exception as e:
    ...
    async def _verify_no_zombie_processes_async(self, initial_pid: int):
        ...
        try:
            ...
            zombies = [child for child in children if child.status() == psutil.STATUS_ZOMBIE]
            if zombies:
                self.logger.error(f"CRITICAL: Found zombie processes related to PID {initial_pid}: {zombies}")
                ...
                self.logger.debug(f"✓ No zombie processes found for PID {initial_pid} and its descendants.")
        except psutil.NoSuchProcess:
            self.logger.debug(f"Parent process {initial_pid} already gone, cannot check for zombies.")
        except Exception as e:
            self.logger.error(f"Error verifying zombie processes for PID {initial_pid}: {e}")
    ...
    def is_worker_healthy(self, worker: WorkerProcess) -> bool:
        ...
        return not self._is_port_available(worker.port)  # Port should NOT be available if worker is running
    ...
    def get_healthy_worker_count(self) -> int:
        ...
        return sum(1 for worker in self.workers.values() if self.is_worker_healthy(worker))
