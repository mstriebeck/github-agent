...
def cmd_list(args):
    ...
    if not config_path.exists():
        ...
        print(
            "Use 'repository_cli.py init --example' to create a sample configuration."
        )
        return
            ...
            for repo_name in repositories:
                ...
                if repo_info:
                    ...
                    if repo_config and repo_config.port:
                        ...
                        print(
                            f"     URL: http://localhost:8080/mcp/{repo_name}/ (legacy single-port mode)"
                        )
                    print()
    else:
        ...
        for repo_name in repositories:
            ...
            if repo_info:
                if repo_info["exists"]:
                    ...
                    print(
                        f"  ❌ {repo_name}: Path does not exist - {repo_info['path']}"
                    )
                    all_valid = False
            else:
    ...
    try:
        ...
        if settings_file.exists():
            try:
                ...
                print(
                    f"Warning: Could not parse existing {settings_file}, creating new settings"
                )
                settings = {}
...
def cmd_setup_vscode(args):
    ...
    for repo_name, repo_config in config_data["repositories"].items():
        ...
        if not repo_port:
            print(
                f"  Skipping {repo_name}: no port assigned (run 'assign-ports' first)"
            )
            continue
...
Examples:
  ...
  repository_cli.py init --example                    # Create example config
    ...
    parser_validate = subparsers.add_parser(
        "validate", help="Validate repository configuration"
    )
    ...
    parser_init.add_argument(
        "--example", action="store_true", help="Create example configuration"
    )
    parser_init.add_argument(
    ...
    parser_assign_ports = subparsers.add_parser(
        "assign-ports", help="Assign ports to repositories for multi-port architecture"
    )
    parser_assign_ports.add_argument(
        "--start-port",
        type=int,
        default=8081,
        help="Starting port number (default: 8081)",
    )
    parser_assign_ports.set_defaults(func=cmd_assign_ports)

    # Status command
    parser_status = subparsers.add_parser(
        "status", help="Show status of master and worker processes"
    )
    parser_status.set_defaults(func=cmd_status)

    # Setup VSCode command
    parser_setup_vscode = subparsers.add_parser(
        "setup-vscode", help="Configure .vscode/settings.json for all repositories"
    )
