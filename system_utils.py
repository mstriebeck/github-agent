...
class MicrosecondFormatter(logging.Formatter):
    ...
    def formatTime(self, record, datefmt=None):  # noqa: N802
        ...
        return dt.strftime("%Y-%m-%d %H:%M:%S.%f")[
            :-3
        ]  # Keep 3 decimal places (milliseconds)
...
def get_system_state():
    ...
    try:
        ...
        system_state = {
            ...
            "children": child_info,
            "threads": {"count": threading.active_count(), "details": thread_info},
...
def log_system_state(logger, phase):
    ...
    logger.debug(
        f"=== SYSTEM STATE: {phase} (at {start_time.strftime('%H:%M:%S.%f')[:-3]}) ==="
    )
    ...
    try:
        ...
        logger.debug(
            f"Memory: RSS={system_state['memory']['rss_mb']:.1f}MB, VMS={system_state['memory']['vms_mb']:.1f}MB"
        )
        ...
        for child in children:
            logger.debug(
                f"  Child PID {child['pid']}: {child['name']} ({child['status']})"
            )
        ...
        for thread in thread_info["details"]:
            logger.debug(
                f"  Thread: {thread['name']} (alive: {thread['alive']}, daemon: {thread['daemon']})"
            )
