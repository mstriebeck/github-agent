...
class TestSwiftLintViolationExtraction(unittest.TestCase):
    ...
    def test_extract_file_from_violation(self):
        ...
        for violation, expected_file in zip(self.sample_violations, expected_files):
    ...
    def test_extract_line_number_from_violation(self):
        ...
        for violation, expected_line in zip(self.sample_violations, expected_line_numbers):
            with self.subTest(violation=violation):
    ...
    def test_extract_severity_from_violation(self):
        ...
        for violation, expected_severity in zip(self.sample_violations, expected_severities):
            with self.subTest(violation=violation):
    ...
    def test_extract_message_from_violation(self):
        ...
        for violation, expected_message in zip(self.sample_violations, expected_messages):
            with self.subTest(violation=violation):
    ...
    def test_extract_rule_from_violation(self):
        ...
        for violation, expected_rule in zip(self.sample_violations, expected_rules):
            with self.subTest(violation=violation):
    ...
    def test_extract_from_partially_valid_lines(self):
        ...
        self.assertEqual(extract_file_from_violation(line_with_non_numeric), "/path/to/file.swift")
        self.assertEqual(extract_line_number_from_violation(line_with_non_numeric), 0)  # No numeric match
        self.assertEqual(extract_severity_from_violation(line_with_non_numeric), "error")
        ...
        self.assertEqual(extract_severity_from_violation(info_line), "")  # 'info' not captured
        self.assertEqual(extract_message_from_violation(info_line), "")  # No rule parentheses
        self.assertEqual(extract_rule_from_violation(info_line), "")  # No rule parentheses
        
        # Test relative path
        relative_line = "relative/path.swift:25:1: warning: message (rule)"
        self.assertEqual(extract_file_from_violation(relative_line), "")  # Absolute path required
        self.assertEqual(extract_line_number_from_violation(relative_line), 0)  # Needs absolute path
        self.assertEqual(extract_severity_from_violation(relative_line), "warning")  # Still matches
        self.assertEqual(extract_rule_from_violation(relative_line), "rule")  # Still matches
...
class TestBuildOutputPatterns(unittest.TestCase):
    ...
    def setUp(self):
        ...
        import re
        self.compiler_error_pattern = re.compile(r'^(/.*\.swift):(\d+):(\d+): error: (.+)$')
        self.compiler_warning_pattern = re.compile(r'^(/.*\.swift):(\d+):(\d+): warning: (.+)$')
        self.test_failure_pattern = re.compile(r'^(/.*\.swift):(\d+): error: (.+) : (.+)$')
    ...
    def test_compiler_error_pattern(self):
        ...
        expected_matches = [
            ("/Users/dev/MyProject/Sources/MyApp/ContentView.swift", "42", "23", "Use of unresolved identifier 'unknownVariable'"),
            ("/path/to/project/Sources/Core/Model.swift", "15", "1", "Expected declaration"),
            ("/absolute/path/MyProject/Sources/Utils/Helper.swift", "89", "45", "Cannot convert value of type 'String' to expected argument type 'Int'")
        ]
        
        for error_line, expected_match in zip(self.sample_compiler_errors, expected_matches):
    ...
    def test_compiler_warning_pattern(self):
        ...
        expected_matches = [
            ("/Users/dev/MyProject/Sources/MyApp/ViewController.swift", "67", "12", "Initialization of immutable value 'result' was never used"),
            ("/path/to/project/Sources/Core/DataManager.swift", "234", "8", "Variable 'temp' was never mutated; consider changing to 'let' constant")
        ]
        
        for warning_line, expected_match in zip(self.sample_compiler_warnings, expected_matches):
    ...
    def test_test_failure_pattern(self):
        ...
        expected_matches = [
            ("/Users/dev/MyProject/Tests/MyAppTests/ContentViewTests.swift", "25", "testButtonTap()", "XCTAssertEqual failed: (\"Expected\") is not equal to (\"Actual\")"),
            ("/path/to/project/Tests/CoreTests/ModelTests.swift", "89", "testDataValidation()", "XCTAssertTrue failed")
        ]
        
        for test_line, expected_match in zip(self.sample_test_failures, expected_matches):
