...
from tests.test_abstracts import MockProcessRegistry, MockSignal, ProcessState
...
class TestIntegratedShutdownFlow:
    ...
    @pytest.mark.asyncio
    async def test_clean_shutdown_cooperative_workers(
        self,
        shutdown_manager,
        process_registry,
        health_monitor,
        ...
        worker1 = process_registry.create_cooperative_process(
            "worker1", shutdown_delay=0.1
        )
        worker2 = process_registry.create_cooperative_process(
            "worker2", shutdown_delay=0.1
        )
        ...
        client1 = process_registry.create_cooperative_client(
            "client1", "worker1", disconnect_delay=0.05
        )
        client2 = process_registry.create_cooperative_client(
            "client2", "worker2", disconnect_delay=0.05
        )
    ...
    @pytest.mark.asyncio
    async def test_shutdown_with_timeout_escalation(
        self,
        shutdown_manager,
        process_registry,
        health_monitor,
        ...
        worker2 = process_registry.create_unresponsive_process(
            "worker2", kill_resistant=False
        )
        ...
        with (
            ...
            async def mock_cleanup_resources_impl():
                try:
                    ...
                    if not port2.release():
                        ...
                        port2.force_release()
                        exit_code_manager.report_force_action(
                            "force_release", "port 8081"
                        )
            ...
            mock_disconnect_clients.return_value = (
                True  # Clients disconnect successfully
            )
    ...
    @pytest.mark.asyncio
    async def test_shutdown_with_zombie_processes(
        self,
        shutdown_manager,
        process_registry,
        health_monitor,
        ...
        # Setup health monitoring
        health_monitor.update_worker_status(
            "zombie_worker", zombie_worker.pid, 8080, "running"
        )
    ...
    @pytest.mark.asyncio
    async def test_shutdown_health_monitoring_integration(
        self, shutdown_manager, process_registry, health_monitor, mock_logger
    ):
        ...
        with (
            ...
            async def mock_stop_workers_impl(grace_period, force_timeout):
                ...
                worker1.wait_for_exit(1.0)
                health_monitor.set_worker_shutdown_completed("worker1")
                health_monitor.update_shutdown_progress(
                    "workers", {"stopped": 1, "total": 1}
                )
                return True
            ...
            async def mock_disconnect_clients_impl():
                ...
                health_monitor.set_client_disconnected("client1")
                health_monitor.update_shutdown_progress(
                    "clients", {"disconnected": 1, "total": 1}
                )
                return True
    ...
    async def test_concurrent_shutdown_attempts(
        ...
        with (
            ...
            results = await asyncio.gather(
                shutdown_task1, shutdown_task2, return_exceptions=True
            )
    ...
    async def test_partial_failure_recovery(
        ...
        bad_worker = process_registry.create_unresponsive_process(
            "bad_worker", kill_resistant=True
        )
        ...
        with (
            ...
            async def mock_stop_workers_impl(grace_period, force_timeout):
                ...
                if bad_worker.is_alive():
                    ...
                    if not success:
                        exit_code_manager.report_system_error(
                            "worker_manager", Exception("Kill-resistant process")
                        )
