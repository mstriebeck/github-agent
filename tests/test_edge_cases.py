...
class TestHealthMonitoringEdgeCases:
    ...
    def test_concurrent_health_updates(self, monitor, mock_logger):
        ...
        def update_worker(thread_id):
            for i in range(10):
                monitor.update_worker_status(
                    f"worker{thread_id}_{i}", 1000 + i, 8000 + i, "running"
                )
        ...
        assert (
            monitor.get_current_status()["workers_count"] == 30
        )  # 3 threads * 10 workers
        assert (
            monitor.get_current_status()["errors_count"] >= 20
        )  # At least some errors (may be truncated)
...
class TestExitCodeEdgeCases:
    ...
    def test_mixed_issue_precedence(self, manager):
        ...
        manager.report_system_error(
            "coordinator", Exception("error")
        )  # System error (40s)
...
class TestProcessLifecycleEdgeCases:
    ...
    def test_process_dies_during_shutdown(self, process_registry):
        ...
        worker = process_registry.create_cooperative_process(
    ...
    def test_kill_resistant_process(self, process_registry):
        ...
        resistant = process_registry.create_unresponsive_process(
            "resistant1", kill_resistant=True
        )
