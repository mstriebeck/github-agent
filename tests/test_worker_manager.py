...
class TestWorkerManager(unittest.TestCase):
    ...
    def setUp(self):
        self.logger = logging.getLogger(
            f"test_worker_manager_{int(time.time() * 1000000)}"
        )
    ...
    async def async_test_shutdown_single_worker_graceful(self, mock_port_check):
        ...
        with patch.object(
            self.manager,
            "_send_worker_shutdown_request",
            side_effect=mock_shutdown_request,
        ):
            result = await self.manager._shutdown_single_worker_async(
                "test-repo", worker, 1, 1
            )
    ...
    async def async_test_shutdown_all_workers(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.manager.get_worker_count(), 0
