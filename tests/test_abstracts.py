...
class CooperativeMockProcess(AbstractMockProcess):
    ...
    def _start_graceful_shutdown(self):
        ...
        self._shutdown_thread = threading.Thread(
            target=self._graceful_shutdown_worker, daemon=True
        )
...
class CooperativeMockPort(AbstractMockPort):
    ...
    def release(self) -> bool:
        ...
        self._release_thread = threading.Thread(
            target=self._delayed_release, daemon=True
        )
...
class CooperativeMockClient(AbstractMockClient):
    ...
    def disconnect_gracefully(self, timeout: float) -> bool:
        ...
        self._disconnect_thread = threading.Thread(
            target=self._delayed_disconnect, daemon=True
        )
...
class MockProcessRegistry:
    ...
    def create_cooperative_process(
        self, name: str, shutdown_delay: float = 0.1
    ...
    def create_unresponsive_process(
        self, name: str, kill_resistant: bool = False
    ...
    def create_cooperative_port(
        self, port: int, release_delay: float = 0.05
    ) -> CooperativeMockPort:
    ...
    def create_sticky_port(
        self, port: int, force_releasable: bool = True
    ) -> StickyMockPort:
    ...
    def create_cooperative_client(
        self, client_id: str, worker_id: str, disconnect_delay: float = 0.1
    ) -> CooperativeMockClient:
        """Create a cooperative mock client."""
        client = CooperativeMockClient(client_id, worker_id, disconnect_delay)
        self.clients[client_id] = client
        return client

    def create_unresponsive_client(
        self, client_id: str, worker_id: str
    ) -> UnresponsiveMockClient:
